    ...
    >
      ...
      <td className='px-3 py-3 text-center tabular-nums text-sm dark:text-white'>
        {(() => {
          ...
          return <span>{isTokenListed() ? <ReportGmailerrorredIcon sx={{ fontSize: 16 }} className="text-gray-400" /> : `$${formatPrice(0)}`}</span>;
        })()}
      </td>

      {/* AT(L-H) (ATL màu đỏ - ATH màu xanh) giống Airdrop */}
      {showATH && (
        <td className='px-3 py-3 text-center tabular-nums text-sm dark:text-white'>
          <span>
